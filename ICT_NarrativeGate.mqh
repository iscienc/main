...
bool NAR_HasRequiredNarrativeAtEntry(const SSMInstance &inst)
  {
   ...
   if(entryDir == DIR_NONE) entryDir = inst.direction;

   ENUM_TF_LAYER eLay = g_smStageCfg[SM_MAX_STAGES - 1].primaryTF;
   int tag = -1; double px = 0.0;

   if(SM_Detect_OB (eLay, entryDir, false, tag, px)) return true;
   if(SM_Detect_FVG(eLay, entryDir, false, tag, px)) return true;
   if(SM_Detect_Breaker   (eLay, entryDir, false, tag, px)) return true;
   if(SM_Detect_Mitigation(eLay, entryDir, false, tag, px)) return true;
   if(SM_Detect_OTE(eLay, entryDir))                 return true;
   return false;
  }
...
bool NAR_IsChainTradable(const SSMInstance &inst, string &reason)
  {
   reason = "";
   if(!inst.active) { reason = "inactive_chain"; return false; }

   // ★ FIX 1: entry stage must be complete
   if(!inst.stageDone[SM_MAX_STAGES - 1])
     { reason = "entry_not_done"; return false; }
   ...
   for(int s = 0; s < SM_MAX_STAGES; s++)
     {
      if(g_smStageCfg[s].required &&
         ...
         !inst.stageDone[s])
        { reason = "required_stage_incomplete"; return false; }
     }

   int age = (inst.stageBarCtr[0] > 0) ? (g_smBarCounter - inst.stageBarCtr[0]) : 0;
   if(age > NAR_MaxChainAgeBars()) { reason = "chain_timeout"; return false; }
   ...
   if(!inst.isCounterDirPreset && !NAR_DirectionAligned(checkDir))
     { reason = "direction_mismatch"; return false; }

   if(!NAR_EnvironmentOK(inst.direction))     { reason = "environment_filter"; return false; }
   if(!NAR_HasRequiredNarrativeAtEntry(inst)) { reason = "no_entry_element";   return false; }
   return true;
  }
//+------------------------------------------------------------------+
//|                                                                  |
